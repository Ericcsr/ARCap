...
def set_system_pose(hand, arm, arm_q, hand_q, handedness):
...
def read_data(path, target_path, delta_orn, delta_pos, start_idx=0, end_idx=0):
    ...
    for i,file in enumerate(files):
        if i < start_idx and i >= len(files) - end_idx:
            continue
        ...
        # Set robot state
        hand_xyz = set_system_pose(left_hand, left_arm, data[f"arm_q"][:7], data["raw_hand_q"][:2], handedness="left")
        hand_xyz = set_system_pose(right_hand, right_arm, data[f"arm_q"][7:], data["raw_hand_q"][2:], "right")
        ...
        if first:
            ...
            world_pcd = crop_pcd(world_pcd, (-0.7,0.1, 0.035), (-0.1,0.6,0.4))
            robot_pcd = crop_pcd(robot_pcd, (-0.7,0.1, 0.035), (-0.1,0.6,0.4))
        ...
        np.savetxt(f"{target_path}/frame_{i}/arm_joints.txt", data[f"arm_q"], delimiter=",")
        np.savetxt(f"{target_path}/frame_{i}/hand_joints.txt", np.atleast_1d(data[f"hand_q"]), delimiter=",")
    if args.visualize:
        vis.destroy_window()
...
right_arm = pb.loadURDF("assets/franka_arm/panda_leap.urdf", basePosition=[0.0, 0.0, 0.0], baseOrientation=[0, 0, 0.7071068, 0.7071068], useFixedBase=True)
right_hand = pb.loadURDF("assets/leap_hand/robot_pybullet.urdf")
left_arm = pb.loadURDF("assets/franka_arm/panda_gripper.urdf", basePosition=[0.0, 0.0, 0.0], baseOrientation=[0, 0, 0.7071068, 0.7071068], useFixedBase=True)
left_hand = pb.loadURDF("assets/gripper/franka_panda_tri_gripper.urdf")

for j, root_path in enumerate(args.root_path):
    robot_frame = np.load(f"data/{root_path}/RobotFrame.npy")
    pb.resetBasePositionAndOrientation(left_arm, robot_frame[:3], (Rotation.from_quat(robot_frame[3:])*Rotation.from_quat([0, 0, 0.7071068, 0.7071068])).as_quat())
    print("Processing root path:", root_path)
    folders = os.listdir(f"data/{root_path}")
    folders.remove("WorldFrame.npy")
    folders.remove("RobotFrame.npy")
    folders.sort()
    ...
    for i,folder in enumerate(folders):
        ...
        read_data(f"data/{root_path}/{folder}", target_path, delta_orn, delta_pos, args.start_idx, args.end_idx)
